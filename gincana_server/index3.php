<?php
    ...
    if ($_SERVER['REQUEST_METHOD']=='POST'){
      if ($_POST['nome'] != null && $_POST['email'] != null){
        ...
        $sql = "INSERT INTO usuario (nome, codigo, email, nascimento, instagram, twitter, facebook, whatsapp, pontos) VALUES (?,?,?,?,?,?,?,?,?)";
        $stmt = $link->prepare($sql);
        if($stmt){
            if($stmt->bind_param('ssssssssi', $nome, $codigo, $email, $data_nascimento, $instagram, $twitter, $facebook, $whatsapp, $pontos)){
                $stmt->execute();
                $user = mysqli_query($link,'SELECT id, nome, pontos, codigo FROM usuario where id = "'.$link->insert_id.'";')->fetch_assoc();
                $return['login'] = [
                    'id' => $user['id'],
                    'status' => 1,
                    'usuario' => $user['nome'],
